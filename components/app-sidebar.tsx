...
const SidebarLink = React.memo(
  ({
    ...
    <Link
      href={href}
      prefetch={true}
      className={`flex items-center justify-between h-10 px-3 py-2 rounded-md text-sm ${isActive ? "fixed-sidebar-item-active" : "fixed-sidebar-item"
        }`}
    >
  ...
  ({
    ...
    <div className="space-y-1 mt-2">
      <button
        ...
        className={`w-full flex items-center justify-between px-3 py-2 text-sm font-medium rounded-md ${activeCheck
...
export function AppSidebar() {
  ...
  const routes: SidebarRoutes = useMemo(
    () => ({
      ...
      companies: {
        ...
        items: [
          { href: "/companies", icon: Buildings, label: translations.companies?.list || "Companies" },
          { href: "/my-company", icon: Buildings, label: translations.myCompany?.title || "MyCompany" },
        ],
      },
    }),
    [
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 bottom-0 z-30 w-64 fixed-sidebar border-r transition-transform md:static ${isSidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
          }`}
      >
