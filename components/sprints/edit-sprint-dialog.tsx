...
import { updateSprint, addSprint } from "@/lib/redux/features/sprints-slice"
...
export function EditSprintDialog({ sprintId, open, onOpenChange }: EditSprintDialogProps) {
  const dispatch = useDispatch()
  const sprint = useSelector((state: RootState) =>
    sprintId === "new" ? null : state.sprints.sprints.find((s) => s.id === sprintId),
  )
  ...
  useEffect(() => {
    ...
    } else if (sprintId === "new") {
      // Set default values for new sprint
      setFormData({
        name: "",
        startDate: today,
        endDate: twoWeeksLater,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>{sprintId === "new" ? "Create New Sprint" : "Edit Sprint"}</DialogTitle>
            <DialogDescription>
              {sprintId === "new" ? "Plan your next development cycle." : "Update sprint details."}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button type="submit" className="bg-[var(--fixed-primary)] text-white">
              {sprintId === "new" ? "Create Sprint" : "Save Changes"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
