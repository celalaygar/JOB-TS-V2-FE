...
export function EditRoleDialog({ open, onOpenChange, role, permissionList, projectId }: EditRoleDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const updatedRole: ProjectRoleRequest = {
      ...role,
      projectId,
      name: roleName.trim(),
    ...
    let response: ProjectRole | null = await saveProjectUserRole(updatedRole);
    if (!response) {
      return;
    }
  ...
  const saveProjectUserRole = async (newRole: ProjectRoleRequest) => {
    ...
    setLoading(true)
    try {
      const response: ProjectRole = await BaseService.request(PROJECT_USER_ROLES_URL + "/" + role?.id, {
      ...
      if (error.status === 400 && error.message) {
        toast({
          title: `Project User Role edit failed. (400)`,
        ...
        toast({
          title: `Project User Role failed.`,
          description: error.message,
          variant: "destructive",
        })
      }
